...
from app.models.security import LoginAttempt, PasswordHistory, TokenBlacklist, UserSession
...
async def check_password_history(user_id: str, new_password: str, db: AsyncSession) -> bool:
    ...
    try:
        ...
        result = await db.execute(
            ...
            .where(PasswordHistory.user_id == user_id, PasswordHistory.is_active.is_(True))
        ...
        for old_password in recent_passwords:
            ...
            if (
                ...
                and verify_password(new_password, password_hash)
            ):
                return False

        return True
    except Exception as e:
        raise AuthenticationException(f"Failed to check password history: {str(e)}") from e
...
async def record_password_history(user_id: str, password_hash: str, db: AsyncSession) -> None:
    ...
    try:
        ...
        raise AuthenticationException(f"Failed to record password history: {str(e)}") from e
...
def create_token(data: dict, expires_delta: timedelta) -> str:
    ...
    to_encode.update(
        {
            ...
            "sub": data.get("sub"),
            "type": "access"
            if expires_delta == timedelta(minutes=int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", 15)))
            else "refresh",
        }
    )
    ...
    try:
        ...
        raise TokenException(f"Failed to create token: {str(e)}") from e


async def verify_token(token: str, db: AsyncSession) -> dict | None:
    """Verify JWT token"""
    try:
        secret_key = os.getenv("SECRET_KEY")
        if secret_key is None:
        ...
        raise TokenException(f"Token verification failed: {str(e)}") from e
...
async def check_token_revocation(token_id: str, db: AsyncSession) -> bool:
    ...
    try:
        ...
        try:
            ...
            raise TokenException(f"Failed to check token revocation: {str(e)}") from e
...
async def revoke_tokens(token: str, user_id: str, db: AsyncSession) -> None:
    ...
    try:
        ...
        raise TokenException(f"Invalid token: {str(e)}") from e
    except Exception as e:
        raise TokenException(f"Failed to revoke token: {str(e)}") from e
    ...
    try:
        ...
        result = await db.execute(
            ...
            .where(
                ...
                LoginAttempt.created_at > datetime.now(UTC) - timedelta(minutes=5),
        ...
        if recent_attempts >= max_attempts:
            ...
            await db.execute(
                ...
                .where(LoginAttempt.user_id == user_id, LoginAttempt.ip_address == ip_address)
        ...
        raise AuthenticationException(f"{str(e)}") from e


async def record_login_attempt(
    user_id: str, ip_address: str, success: bool, db: AsyncSession
) -> None:
    """Record login attempt"""
    try:
        attempt = LoginAttempt(user_id=user_id, ip_address=ip_address, success=success)
        db.add(attempt)
        await db.commit()
    except Exception as e:
        raise AuthenticationException(f"Failed to record login attempt: {str(e)}") from e


async def create_user_session(
    user_id: str, ip_address: str, user_agent: str | None, db: AsyncSession
) -> str:
    ...
    try:
        ...
        session = UserSession(
            user_id=user_id, session_id=session_id, ip_address=ip_address, user_agent=user_agent
        )
        db.add(session)
        await db.commit()
        ...
        raise SessionException(f"Failed to create user session: {str(e)}") from e
    ...
    try:
        result = await db.execute(
            select(UserSession).where(
                UserSession.session_id == session_id, UserSession.is_active.is_(True)
            )
        )
        ...
        raise SessionException(f"Failed to update session activity: {str(e)}") from e
...
async def deactivate_session(session_id: str, db: AsyncSession) -> None:
    ...
    try:
        ...
        structlog.get_logger(__name__).warning("deactivate_session_failed", error=str(e))
